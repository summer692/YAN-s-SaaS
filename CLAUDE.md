...
```js
// service-worker.js
const ATLAS_VERSION = 'atlas-vNN';  // 每次推送前数字 +1（vNN 是示例占位，不是当前值）

// app.js
const ATLAS_VERSION = 'atlas-vNN';  // 必须和上面完全一致
```
...
| `notes` | text | |
| `created_at`, `updated_at`, `deleted_at` | timestamptz | `deleted_at` 非空 = 在废纸篓。`created_at` / `updated_at` 由 Supabase 默认值/触发器写入，`projectToRow` **不**回写这两列，只回写 `deleted_at` |
...
| `priority` | text | `低` / `中` / `高` |
| `created_at`, `deleted_at` | timestamptz | `created_at` 由 Supabase 默认值写入，`ideaToRow` **不**回写它，只回写 `deleted_at` |
...
用户问起任何"那个脚本怎么跑"、"周报在哪看"、"我之前装的那个东西"，从这一节查。

### 1. 灵感周报：每周五早 8 点自动生成
- **是什么**：拉 Supabase 所有 ideas，让 Claude 做合并 / 归档 / Top 商业化分析 / 没看明白单列。
- **脚本**：`/Users/celialuo/dev/atlas/scripts/idea-refinery.sh`
- **手动跑**：终端打 `~/dev/atlas/scripts/idea-refinery.sh`
- **自动调度**：`~/Library/LaunchAgents/com.celialuo.atlas.refinery.plist`，每周五 8am 跑；Mac 睡着会在下次醒来自动补跑。
- **看产出**：**脚本跑完会自动用 VS Code 打开 digest**（无需用户操作）。备用：终端打 `digest` alias 也能打开最新一份。VS Code 里按 `Cmd+Shift+V` 看 Markdown 渲染。
- **产出目录**：`~/atlas-digests/`，文件名 `refinery_YYYY-MM-DD_HHMM.md`，日志 `launchd.log`。
- **停用**：`launchctl unload ~/Library/LaunchAgents/com.celialuo.atlas.refinery.plist`
- **依赖**：本地代理 `127.0.0.1:6666`（FlclientC 或其它科学上网工具）必须在跑，否则 claude -p 会 403。

### 2. Subagent：code-reviewer + debugger
- **位置**：`/Users/celialuo/dev/atlas/.claude/agents/`
- **何时自动触发**：每次 Claude 改了 `service-worker.js` / `*.sql` / `config.js` / `vercel.json`，hook 会自动叫 code-reviewer。
- **手动触发**：在 atlas 的 CC 会话里说 "用 code-reviewer 审一下" 或 "让 debugger 查这个 bug"。

### 3. Hook：关键文件改动自动审查
- **位置**：`/Users/celialuo/dev/atlas/.claude/hooks/notify-critical-edit.sh`
- **配置**：`/Users/celialuo/dev/atlas/.claude/settings.json`（项目共享）
- **触发条件**：Claude 写 / 编辑前述 4 类关键文件时。

### 4. MCP：Supabase（read-only + 锁定 atlas project）
- **传输**：HTTP，远程 `https://mcp.supabase.com/mcp`
- **限制**：URL 带 `read_only=true` + `project_ref=<atlas-id>`，**数据库层强制只读**。
- **认证**：Personal Access Token，存在本机 `~/.claude.json`。
- **CC 启动时自动加载**，用户问数据库相关问题时主动调用。
